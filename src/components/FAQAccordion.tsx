      ...
      {faqs.map((item, i) => (
        <div key={i} className="border-b border-[var(--border)]">
          <button
            ...
            className="w-full flex items-center justify-between gap-4 py-5 bg-transparent border-none cursor-pointer text-left font-[inherit] hover:text-[var(--accent)] transition-colors group"
          >
            <span className={`text-[14px] font-medium transition-colors duration-150 ${open === i ? 'text-[var(--accent)]' : 'text-[var(--text)]'}`}>
              {item.q}
            </span>
            <span className={`shrink-0 w-5 h-5 flex items-center justify-center border text-[13px] leading-none transition-all duration-150 ${
              open === i
          ...
          >
            <p className="text-[13px] text-[var(--text-secondary)] leading-[1.8] pb-5 max-w-[580px]">
              {item.a}
            </p>
          </div>
        </div>
